# MetadataExtractor-3/get_image)metadata.py
...
INPUT_DIRECTORY = os.path.join(script_dir, "../testFiles")
OUTPUT_DIRECTORY = os.path.join(script_dir, "outputFolder-3")
METADATA_FORMAT = "txt"  # Choose (txt or xlsx)
LOG_FILE_PATH = os.path.join(OUTPUT_DIRECTORY, "metadata_log.txt")

# Dynamic Padding Configuration
PADDING_LEFT_FACTOR = 0.03
PADDING_RIGHT_FACTOR = 0.75
...
def get_image_metadata(image_path):
    ...
    metadata = {
        ...
        "Origin Date": parse_image_date(str(tags.get('EXIF DateTimeOriginal'))) if tags.get('EXIF DateTimeOriginal') else None
    }
    return metadata
    ...
    with open(LOG_FILE_PATH, "a") as log_file:
        for root, dirs, files in os.walk(INPUT_DIRECTORY):
            for filename in files:
                ...
                if filepath.lower().endswith((".jpg", ".jpeg", ".gif", ".tiff")):
                    metadata = get_image_metadata(filepath)
                    output_path = os.path.join(OUTPUT_DIRECTORY, os.path.splitext(filename)[0] + "_MD.png")
                    write_metadata(metadata, output_path.replace(".png", ".txt"), METADATA_FORMAT)
